...
func getEvent(eventID, loginUserID int64) (*Event, error) {
	...
	rows, err := db.Query("SELECT reservations.*, sheets.* FROM reservations INNER JOIN sheets on reservations.sheet_id = sheets.id WHERE event_id = ? AND canceled_at IS NULL", event.ID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&reservation.ID, &reservation.EventID, &reservation.SheetID, &reservation.UserID, &reservation.ReservedAt, &reservation.CanceledAt, &sheet.ID, &sheet.ID, &sheet.Rank, &sheet.Num, &sheet.Price); err != nil {
			return nil, err
		}
